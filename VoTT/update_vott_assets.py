...
def replace_old_contents(target_directory, old_to_new_ids, old_source_directory,
	...
	files = [f for suffix in ('**/*.vott', '**/*.json') for f in
			 glob.glob(os.path.join(target_directory, suffix), recursive=True) if os.path.isfile(f) == True]
	...
	for byteline in fileinput.input(files=files, inplace=True, mode='rb'):
		try:
			...
			for id_pair in old_to_new_ids.items():
				# the old asset id
				old_id = get_hash(id_pair[0])
				# the new asset id
				new_id = get_hash(id_pair[1])
