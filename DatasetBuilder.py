	...
	try:
		...
		if ImgRequest.status_code == requests.codes.ok:
			...
			if (int(nums1[0]) is not 0) and (int(nums2[0]) is not 0):
				...
				# TODO: When a circle is found, adjust the bounding box to match the (x_min, y_min) (x_max, y_max) screw coordinates
				# TODO: Write the image to a temporary dataset, with an entry added to a temporary bounding box file recording the predicted boxes
				# TODO: Each image has labelled corresponding bounding box list, empty if no bbx or screw edge detected
